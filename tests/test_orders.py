...
def test_create_order_success_persists_live_order(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda api_key, api_secret, symbol, side, quantity: {
...
def test_create_order_rejected_by_exchange_persists_error(monkeypatch):
    ...
    def _raise(api_key, api_secret, symbol, side, quantity):
    ...
    response = client.post(
        f"/brokers/binance/connections/{connection_id}/orders",
        json={"symbol": "BTCUSDT", "side": "sell", "quantity": 100.0},
        headers=_auth_headers(token),
    )
    assert response.status_code == 200
    body = response.json()
...
def test_create_order_rejects_invalid_side(monkeypatch):
    ...
    response = client.post(
        f"/brokers/binance/connections/{connection_id}/orders",
        json={"symbol": "BTCUSDT", "side": "hold", "quantity": 1.0},
        headers=_auth_headers(token),
    )
    assert response.status_code == 400


def test_create_order_rejects_non_positive_quantity(monkeypatch):
    ...
    response = client.post(
        f"/brokers/binance/connections/{connection_id}/orders",
        json={"symbol": "BTCUSDT", "side": "buy", "quantity": 0},
        headers=_auth_headers(token),
    )
    assert response.status_code == 400
...
def test_list_orders_returns_most_recent_first(monkeypatch):
    ...
    monkeypatch.setattr(
        "tradingos.api.routers.brokers.binance_place_market_order",
        lambda api_key, api_secret, symbol, side, quantity: {"exchange_order_id": "1", "raw": {}},
    )
    client.post(
        f"/brokers/binance/connections/{connection_id}/orders",
        json={"symbol": "BTCUSDT", "side": "buy", "quantity": 0.001},
    ...
    client.post(
        f"/brokers/binance/connections/{connection_id}/orders",
        json={"symbol": "ETHUSDT", "side": "sell", "quantity": 0.01},
        headers=_auth_headers(token),
    )
