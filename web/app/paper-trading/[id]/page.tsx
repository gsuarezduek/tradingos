...
async function fetchSession(id: string): Promise<{ session: SessionDetail | null; error: string | null }> {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/paper-trading/sessions/${id}`, {
...
export default async function PaperTradingSessionPage({ params }: { params: Promise<{ id: string }> }) {
